"""
Pipeline final corrige: 3 fautes methodologiques resolues simultanement.
1. Split PlantDoc par IMAGE SOURCE (groupe), pas par crop -> pas de fuite inter-crop.
2. Selection du meilleur epoch sur une validation dediee, jamais sur le test.
3. Le test set n'est evalue qu'UNE SEULE FOIS, a la toute fin.
"""
...
def rebuild_groups(labels_names, name_to_id):
...
def manifold_mixup(X, y, n_classes, n_per_class=150, alpha=0.4, noise_ratio=0.05):
    synth_X, synth_y = [], []
...
def main():
    print("="*70)
    print("ETAPE 1/4 : Reconstruction du split PlantDoc SANS fuite d'image")
    print("="*70)
    ...
    groups, labels_check = rebuild_groups(labels_names, name_to_id)
    y_pd_full = np.load("data/plantdoc_labels.npy")
    X_pd_full = np.load("data/plantdoc_embeddings.npy")
    assert len(groups) == len(y_pd_full) and np.array_equal(labels_check, y_pd_full), "MISMATCH - arret"
    print(f"Coherence verifiee: {len(groups)} crops, {len(np.unique(groups))} images sources")

    # Split 1: train(70%) / test(30%) PAR GROUPE (image)
    ...
    print(f"Verification anti-fuite train/test: {len(overlap)} images partagees (doit etre 0)")
    assert len(overlap) == 0

    # Split 2: dans le train, on isole encore une VAL pour la selection de modele (PAR GROUPE aussi)
    ...
    print(f"Verification anti-fuite train/val: {len(overlap2)} images partagees (doit etre 0)")
    ...
    print(f"\nPlantDoc train (apprentissage): {len(y_pd_train)} crops")
    print(f"PlantDoc val (selection modele): {len(y_pd_modelval)} crops")
    print(f"PlantDoc test (jamais touche avant la fin): {len(y_pd_test)} crops")

    print("\n" + "="*70)
    print("ETAPE 2/4 : Preparation du dataset d'entrainement combine")
    print("="*70)
    ...
    print("Generation d'embeddings synthetiques (manifold mixup) sur PlantDoc TRAIN uniquement...")
    X_synth, y_synth = manifold_mixup(X_pd_train, y_pd_train, n_classes, n_per_class=150)
    print(f"Synthetiques generes: {len(y_synth)}")
    ...
    print("\n" + "="*70)
    print("ETAPE 3/4 : Entrainement (selection sur PlantDoc VAL, jamais le test)")
    print("="*70)
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch+1) % 5 == 0 or epoch == 0:
            print(f"Epoch {epoch+1:2d}/{n_epochs} | loss={avg_loss:.4f} | val_f1(PV)={val_f1:.4f} | val_acc(PlantDoc)={pdval_acc:.4f}")

        if pdval_acc > best_val_acc:
            best_val_acc = pdval_acc
            torch.save(model.state_dict(), "models/final_corrected_best.pt")

    print(f"\nMeilleur modele (selectionne sur PlantDoc VAL, PAS le test): models/final_corrected_best.pt")

    print("\n" + "="*70)
    print("ETAPE 4/4 : EVALUATION FINALE UNIQUE (test jamais vu avant maintenant)")
    print("="*70)

    model.load_state_dict(torch.load("models/final_corrected_best.pt"))
    model.eval()
    with torch.no_grad():
    ...
    print(f"\n*** RESULTAT FINAL HONNETE (3 fautes methodologiques corrigees) ***")
    ...
    print(f"Baseline (PlantVillage only) sur PlantDoc: 0.3746 (fiable, aucune fuite possible)")
    print(f"Mix simple (fuite crop non corrigee): 0.7243")
    print(f"MLP+mixup (fuite crop + selection sur test): 0.8040")
    print(f"MLP+mixup CORRIGE (sans aucune fuite): {pd_acc:.4f}  <-- CHIFFRE A CITER")
    ...
    for tc in range(n_classes):
        for pc in range(n_classes):
            if tc != pc and cm[tc, pc] > 0:
                errors.append((cm[tc, pc], labels_names[tc], labels_names[pc]))
    errors.sort(reverse=True)
    print("\nTop 10 confusions (test set propre):")
    for count, tn, pn in errors[:10]:
        print(f"  {tn:<45} -> {pn:<45} : {count} erreurs")
